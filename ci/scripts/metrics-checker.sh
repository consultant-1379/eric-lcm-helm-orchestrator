...
METRICS_DESINGE_RULES="https://eteamspace.internal.ericsson.com/display/AA/ADP+Prometheus+Metrics+Collection+Design+Rules#ADPPrometheusMetricsCollectionDesignRules-DR-D470223-010";
...
checkCoreApplicationJAVA(){
    ...
    find ./eric-lcm-helm-executor-server/src/main/java -regex '.*Application.java' -exec grep -o 'public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder)' {} \; > .bob/var.scrapedLine || true;
    find ./eric-lcm-helm-executor-server/src/main/java -regex '.*Application.java' -exec grep -o 'return restTemplateBuilder.build()' {} \; >> .bob/var.scrapedLine || true;
    ...
    if cmp -s .bob/var.compareToLine .bob/var.scrapedLine; then
        ...
        echo -e "Please also ensure your main java file found in './eric-lcm-helm-executor-server/src/main/java' follows the naming convention '<service-name>Application.java'"
        echo -e "What is needed:"
        cat .bob/var.compareToLine
        echo -e "\nWhat was provided:"
        cat .bob/var.scrapedLine
