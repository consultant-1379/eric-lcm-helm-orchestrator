...
Contract.make {
    description("""
Represents a successful scenario of Get all backups details by backup manager id
...
""")
    request {
        method 'GET'
        url"/cnwlcm/v1/backup_and_restore/backupManagerId"
    }
    response {
        status OK()
        body(file("all-backups.json"))
        bodyMatchers {
            ...
            jsonPath('$._links.prev.href', byCommand("assertThat(parsedJson.read(\"\$._links.prev\", Object.class)).isNull()"))
            jsonPath('$._links.self.href', byCommand("assertThat(parsedJson.read(\"\$._links.self\", Object.class)).isNotNull()"))
            jsonPath('$._links.next.href', byCommand("assertThat(parsedJson.read(\"\$._links.next\", Object.class)).isNull()"))
