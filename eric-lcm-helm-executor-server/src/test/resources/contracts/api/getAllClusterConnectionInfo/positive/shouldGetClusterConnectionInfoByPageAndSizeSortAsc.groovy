...
package contracts.api.getAllClusterConnectionInfo.positive

import org.springframework.cloud.contract.spec.Contract

Contract.make {
    description("""
Represents a successful scenario of listing cluster connection info with pagination

```
given:
  client requests to view cluster connection info on 2nd page (page numbers start from 1) with page size of 2 and in ascending order sorted by name
when:
  a valid request is made
then:
  connection info specified are displayed.
```
...
""")
    ...
    response {
        ...
        body(file('pageTwoClusterConnectionInfoAsc.json'))
        bodyMatchers {
