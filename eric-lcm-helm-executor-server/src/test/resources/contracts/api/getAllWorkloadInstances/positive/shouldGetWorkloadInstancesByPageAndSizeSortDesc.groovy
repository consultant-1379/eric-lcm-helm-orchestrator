...
```
given:
  client requests to view Workload Instances on 2nd page (page numbers start from 1) with page size of 2 and in descending order sorted by 
  workloadInstanceId
when:
...
""")
    request {
        ...
        urlPath("/cnwlcm/v1/workload_instances") {
            queryParameters {
                parameter 'page': '2'
                parameter 'size': '2'
                parameter 'sort': value(consumer(regex(/(workloadInstanceId,desc)/)), producer("workloadInstanceId,desc"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file('pageTwoWorkloadInstancesDesc.json'))
        bodyMatchers {
