...
```
given:
  client requests to view Operations on 2nd page (page numbers start from 1) with page size of 2 and in descending order sorted by startTime
when:
...
""")
    request {
        ...
        urlPath("/cnwlcm/v1/operations/workload_instances/workload_instanceId") {
            queryParameters {
                parameter 'page': '2'
                parameter 'size': '2'
                parameter 'sort': value(consumer(regex(/(startTime,desc)/)), producer("startTime,desc"))
            }
        }
        headers {
    ...
    response {
        ...
        body(file('pageTwoOperationsDesc.json'))
        bodyMatchers {
